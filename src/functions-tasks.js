...
function getPolynom(...args) {
  return function sd(arg) {
    let answ = 0;
    for (let i = 0; i < args.length; i += 1) {
      answ += args[i] * arg ** (args.length - i - 1);
    }
    return answ;
  };
}
...
function logger(func, logFunc) {
  return function ads(...args) {
    const temp = args.length > 1 ? JSON.stringify(args).slice(1, -1) : args;
